...
from notebooks.Strategies.CryptoPairTradingV1.backtest.pair_strategy import PairTradingStrategy
...
symbols = ['BTC', 'ETH', 'ADA', 'BNB', 'SOL', 'XRP', 'DOGE', 'DOT', 'LUNA', 'UNI', 'LINK', 'BCH', 'LTC', 'ALGO', 'AVAX',
           'ICP', 'FTT', 'MATIC', 'FIL', 'XLM', 'VET', 'ETC']
symbols = [f'{s}-USDT' for s in symbols]


def read_data(symbol):
    df = pd.read_csv(f'{DATA_PATH}/1h/{symbol}.csv',
...
def runstrategy():
    ...
    symbols = ['DOGE-USDT', 'DOT-USDT', 'LUNA-USDT']
    ...
    cerebro.addstrategy(SimplePairTradingStrategy, df=df, names=symbols,
                        hedge_ratio=[5.42541351, -9.04346413, 3.90687746],
                        nb_symbols=3)
