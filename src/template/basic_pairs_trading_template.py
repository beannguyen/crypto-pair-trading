...
def check_pair_status(context, data):
    ...
    prices = data.history(context.stocks, 'price', 35,
                          '1d').iloc[-context.lookback::]
        ...
        if context.spread.shape[1] > context.z_window:
            ...
            if zscore < -1.0 and (not context.inLong[i]):
                ...
                (y_target_pct, x_target_pct) = computeHoldingsPct(
                    y_target_shares, X_target_shares, Y[-1], X[-1])

                context.target_weights[stock_y] = y_target_pct * \
                    (1.0/context.num_pairs)
                context.target_weights[stock_x] = x_target_pct * \
                    (1.0/context.num_pairs)
...
def get_current_portfolio_weights(context, data):
    ...
    current_weights = share_counts * current_prices / \
